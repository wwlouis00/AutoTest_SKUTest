...
def start_process(start_button,stop_button,local_folder):
    ...
    # 取得選中的 Radiobutton 名稱
    selected_test_case = selected_option.get()
    selected_test_case_name = None

    checkbox_labels = {
        1: "Execute.bat",
        2: "ExecuteTemplate",
        3: "ExecuteTestCase_SQ_AutoQuery(AcAxQuickRelease)",
        4: "ExecuteTestCase_SQ_AutoQuery(AcessDeviceByIp)(AcAxQuickRelease)",
        5: "ExecuteTestCase_SQ_AutoQuery(AcessDeviceByIp)",
        6: "ExecuteTestCase_SQ_AutoQuery",
        7: "ExecuteTestCase_SQ_AutoQuery_for_failed_log(AcAxQuickRelease)",
        8: "ExecuteTestCase_SQ_AutoQuery_for_failed_log",
        9: "ExecuteTestCase_SQ_SkuSetting",
        10: "ExecuteTestCase_UI_AutoQuery"
    }
    
    if selected_test_case in checkbox_labels:
        selected_test_case_name = checkbox_labels[selected_test_case]

    logger.info(f"選中的測試案例: {selected_test_case} {selected_test_case_name}")
     # 按鈕狀態切換
    ...
    thread = threading.Thread(
        target=download_and_process_files, args=(start_button,recipients,local_folder,selected_test_case), daemon=True
    )
    thread.start()
        ...
        for root, _, files in os.walk(directory):
            ...
            if "chromedriver.exe" in files:
                try:
                    ...
                    logger.info(f"Replaced ChromeDriver at: {file_path}")
                except Exception as e:
                    logger.error(f"Failed to replace ChromeDriver: {e}")
...
def monitor_templog_for_matrix(gateway_ip,local_folder, recipients):
    ...
    while not matrix_detected.is_set():
        ...
        if os.path.exists(matrix_file_path):
            ...
            create_zip_and_send_email(gateway_ip,recipients,matrix_file_path)
            matrix_detected.set()
        time.sleep(1)  # Poll every second

def execute_batch_file(gateway_ip,directory,recipients,selected_test_case):
    ...
    try:
        ...
        selected_number = selected_test_case
        # 定義執行命令
        ...
        match selected_number:
            case 1:
                command = (
                    ...
                    'SkuTest ".\\localization\\GT-AXE11000-HwA_V5-10.json" -u Jenkins -db'
                )
            case 2:
                command = (
                    '@echo off & (echo. & echo. & echo. & echo.) | '
                    'SkuTest "LOCALIZATION_JSONS" -db -awbip -dfs cir -u JenkinsATE'
                )
            case 3:
                command = (
                    '@echo off & (echo. & echo. & echo. & echo.) | '
            ...
            case 5:
                command = (
                    '@echo off & (echo. & echo. & echo. & echo.) | '
                    'SkuTest ".\\localization\\Dummy_V5-0.json" -db -aq -awbip -dltl'
                )
            case 6:
                command = (
                    '@echo off & (echo. & echo. & echo. & echo.) | '
            ...
            case 7:
                command = (
                    '@echo off & (echo. & echo. & echo. & echo.) | '
                    'SkuTest ".\\localization\\Dummy_V5-0.json" -db -aq -sels -dltl -reg -rmbl -rmsg'
                )
            case 8:
                command = (
                    '@echo off & (echo. & echo. & echo. & echo.) | '
                    'SkuTest ".\\localization\\Dummy_V5-0.json" -db -aq -sels -dltl'
                )
            case 9:
                command = (
                    '@echo off & (echo. & echo. & echo. & echo.) | '
                    'SkuTest ".\\localization\\Dummy_V5-0.json" -db -aq -ss -dltl'
                )
            case 10:
                command = (
                    '@echo off & (echo. & echo. & echo. & echo.) | '
                    'SkuTest ".\\localization\\Dummy_V5-0.json" -db -aq -dltl -u UI'
                )
        #  ----------------------------------------------------------
        # 啟動監視執行緒
        start_monitor_thread(root, recipients)
...
def create_zip_and_send_email(gateway_ip,recipients,matrix_file_path):
   # 檢查所需檔案是否存在
    # set_ip_metric_for_gateway(gateway_ip, 0)
    source_dir = "TempLog"
    matrix_dir = os.path.dirname(matrix_file_path)
    ...
    if not os.path.exists(matrix_file_path):
        logger.info(f"Directory not found: {matrix_file_path}")
        return
    ...
    # 設定壓縮檔名稱
    zip_file_name = f"TestReport_{CURRENT_TIME}.zip"
    zip_file_path = os.path.join(os.path.dirname(matrix_dir), zip_file_name)

    # 壓縮資料夾
    shutil.make_archive(zip_file_path.replace('.zip', ''), 'zip', matrix_dir)
    ...
    # 設定郵件標題
    subject = f"SKU Test Result Report - {CURRENT_TIME}"
    ...
    #     zip_file_name = f"{found_file_name} {CURRENT_TIME}" + '.zip'
...
def check_stop(ftp=None):
    ...
    return False
# 處理檔案的主函數
def download_and_process_files(start_button,recipients,local_folder,selected_test_case):
    ...
    try:
        ...
        # # **步驟 2: 執行批次檔案**
        # extract_path = execute_batch_file(gateway_ip, local_folder, recipients,selected_test_case)
...
# 按鈕觸發的函數
def on_closing(root):
    save_emails()  # Save email history before closing
    # If there are background threads, we can try to stop them here
    for thread in threading.enumerate():
        if thread != threading.main_thread():
            try:
                thread.join(timeout=1)  # Try to gracefully stop other threads
            except:
                pass  # Ignore any exceptions while stopping threads
...
def main():
    ...
    root.config(bg="#2C3E50")  # 設置背景顏色

    # 定義字型
    bold_font = font.Font(family='Arial', size=14, weight='bold')
    button_font = font.Font(family='Arial', size=14, weight='bold')
    checkbox_font = font.Font(family='Arial', size=14, weight='bold')

    # 建立滾動區域來顯示收件人
    frame_recipients = tk.Frame(root, bg="#333333")
    frame_recipients.pack(pady=10, fill=tk.X)

    # 儲存所有收件人輸入欄位的列表
    email_frame = tk.Frame(root, bg="#34495E")
    ...
    btn_add_recipient = tk.Button(
        email_frame, text="新增收件人", font=button_font, fg="white", bg="#5D6D7E", command=add_recipient
    )
    btn_add_recipient.grid(row=0, column=1, padx=5, pady=5)

    # 歷史收件人按鈕
    history_button = tk.Button(
        email_frame, text="選擇歷史信箱", font=button_font, fg="white", bg="#5D6D7E", command=lambda: open_email_selection(root)
    )
    ...
    checkbox_frame.pack(side=tk.LEFT, fill=tk.Y, padx=5, pady=5)

    selected_option = tk.IntVar()

    label_recipient_list = tk.Label(
        checkbox_frame, text="執行列表", font=bold_font, bg="#222222", fg="white"
    )
    ...
    checkbox_options = [
        ("Execute.bat", 1),
        ("ExecuteTemplate", 2),
        ("ExecuteTestCase_SQ_AutoQuery(AcAxQuickRelease)", 3),
        ("ExecuteTestCase_SQ_AutoQuery(AcessDeviceByIp)(AcAxQuickRelease)", 4),
        ("ExecuteTestCase_SQ_AutoQuery(AcessDeviceByIp)", 5),
        ("ExecuteTestCase_SQ_AutoQuery", 6),
        ("ExecuteTestCase_SQ_AutoQuery_for_failed_log(AcAxQuickRelease)", 7),
        ("ExecuteTestCase_SQ_AutoQuery_for_failed_log", 8),
        ("ExecuteTestCase_SQ_SkuSetting", 9),
        ("ExecuteTestCase_UI_AutoQuery", 10)
    ]

    for text, value in checkbox_options:
        tk.Radiobutton(
            checkbox_frame, text=text, variable=selected_option, value=value,
            font=checkbox_font, bg="#5D6D7E", fg="white", selectcolor="#5D6D7E", anchor='w'
        ).pack(fill=tk.X, padx=5, pady=1)

    # 收件人列表框架
    frame_recipients = tk.Frame(main_frame, bg="#222222", bd=2, relief=tk.GROOVE)
    frame_recipients.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=5, pady=5)

    label_recipient_list = tk.Label(
        frame_recipients, text="收件人列表", font=bold_font, bg="#222222", fg="white"
    )
    label_recipient_list.pack(anchor="w", padx=5, pady=5)

    recipient_list_frame = tk.Canvas(frame_recipients, bg="#333333")
    scrollbar = Scrollbar(frame_recipients, orient="vertical", command=recipient_list_frame.yview)
    # recipient_list_frame.pack(fill=tk.BOTH, expand=True)
    frame_recipients = Frame(recipient_list_frame, bg="#333333")

    frame_recipients.bind(
        "<Configure>", lambda e: recipient_list_frame.configure(
            scrollregion=recipient_list_frame.bbox("all")
        )
    )
    
    recipient_list_frame.create_window((0, 0), window=frame_recipients, anchor="nw")
    recipient_list_frame.configure(yscrollcommand=scrollbar.set)
    
    recipient_list_frame.pack(side="left", fill="both", expand=True)
    ...
    start_button = tk.Button(
        button_frame, text="START", font=bold_font, fg="white", bg="#5cb85c",
    ...
    stop_button = tk.Button(
        button_frame, text="STOP", font=bold_font, fg="white", bg="#f0ad4e",
    ...
    clear_button = tk.Button(
        button_frame, text="CLEAN", font=bold_font, fg="white", bg="#d9534f",
