...
def start_process(start_button,stop_button,local_folder):
    ...
    checkbox_labels = {
        1: "ExecuteTestCase_SQ_SkuSetting",
        2: "ExecuteTestCase_SQ_AutoQuery(AcAxQuickRelease)",
        3: "ExecuteTestCase_SQ_AutoQuery",
        4: "ExecuteTestCase_SQ_SkuSetting",
        5: "ExecuteTestCase_SQ_SkuSetting",
        6: "ExecuteTestCase_UI_AutoQuery"
    }

    if selected_test_case in checkbox_labels:
        selected_test_case_name = checkbox_labels[selected_test_case]
     # 按鈕狀態切換
    logger.info(f"選中的測試案例: {selected_test_case_name}")
...
def download_and_extract(ftp, local_folder):
    ...
    def download_file(ftp, remote_filename, local_path):
        ...
        try:
            ...
            logger.error(f"Download failed for {remote_filename}: {e}")
            if os.path.exists(local_path):
                os.remove(local_path)  # 刪除部分下載的文件
            raise  # 重新拋出異常以便上層捕獲並處理
    ...
    def extract_file(local_path, extract_path):
        ...
        try:
            ...
            if is_valid_extracted(extract_path):
                safe_remove(local_path)  # 成功後刪除壓縮文件
                logger.info(f"Extraction successful: {extract_path}")
            else:
                logger.warning(f"Extraction failed: {extract_path} is empty.")
                raise ValueError(f"Extraction failed: {extract_path} is empty.")
        
        except Exception as e:
            logger.error(f"Extraction failed: {e}")
            # 如果解壓失敗，可以不刪除壓縮包，保持狀態以便再次嘗試
            if os.path.exists(extract_path):
                safe_remove(extract_path)  # 刪除解壓的空資料夾
            raise  # 重新拋出異常，讓上層處理
    ...
    try:
        ...
        for filename in filenames:
            ...
            try:
                ...
                # 下載或解壓失敗時，回退處理：刪除不完全的下載文件
                if os.path.exists(local_path):
                    os.remove(local_path)
                if os.path.exists(extract_path):
                    safe_remove(extract_path)
                continue  # 繼續處理下個文件
            else:
                # 成功後進行後處理
                if is_valid_extracted(extract_path):
...
def create_zip_and_send_email(gateway_ip,recipients):
   # 檢查所需檔案是否存在
    ...
    for root, dirs, files in os.walk(source_dir):
        for file in files:
            ...
            if re.search(file_name_pattern, file):
                ...
                logging.info(f"Found file: {found_file_name}")
                break  # 只抓取第一個符合條件的檔案
    ...
    for root, dirs, files in os.walk(source_dir):
        for file in files:
            # 檢查檔案名稱是否包含 [PASS] 或 [FAIL] 字串
            if re.search(file_name_pattern, file):
                found_file_name = file
                found_file_path = os.path.join(root, file)
                attachments.append(found_file_path)  # 將找到的檔案加入附件列表
                break
        if found_file_name:
            break  # 找到檔案後跳
    
    logger.info(f"找到的檔案: {attachments}")
    ...
    #     send_email(recipients, subject, attachments)
    # else:
    #     logger.info("未找到符合條件的檔案")
...
def main():
    ...
    root.state('zoomed')  # Windows 下的最大化
    ...
    entry_email = tk.Entry(email_frame, font=button_font, width=40, bg="#5D6D7E", fg="white", insertbackground="white")
    ...
    checkbox_options = [
        ...
        ("ExecuteTestCase_SQ_SkuSetting", 4),
        ("ExecuteTestCase_SQ_SkuSetting", 5),
        ("ExecuteTestCase_UI_AutoQuery", 6)
    ]
    ...
    log_text = scrolledtext.ScrolledText(
        root, wrap=tk.WORD, state='disabled', height=20, width=200, bg="#2E3B4E", fg="#A1D6B6", font=("Courier New", 10)
    )
